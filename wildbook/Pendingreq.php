...
$query=mysql_query("SELECT id,name AS friends FROM users WHERE id in(select uid from friends where fid =$id and uid not in(
SELECT X.fid AS fid
...
{
$frid=$_REQUEST['hidden'];
//$fname=$_REQUEST['fhidden'];
//echo $frid;
//echo $fname;
$query="insert into friends values ('$id','$frid')";
mysql_query($query);
				...
				while($row=mysql_fetch_array($query))
					{
						...
						echo"<div>";
							echo"<div>";
								...
								echo"</div>";
								echo "<div>";
								echo'<input type="hidden" name="hidden" value='.$fid.'>';
								//echo'<input type="hidden" name="fhidden" value='.$row['friends'].'>';
