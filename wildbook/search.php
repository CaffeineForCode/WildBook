...
$search=$_SESSION['tsearchid'];
//echo $search;
$squery=mysql_query("Select id,name , content_title, content, times from contents natural join users where uid = id and uid in
(SELECT uid FROM contents
WHERE uid IN ( SELECT X.fid AS fid FROM (
SELECT fid FROM friends WHERE uid =$id )X
INNER JOIN ( SELECT uid FROM friends WHERE fid =$id )Y ON X.fid = Y.uid)
AND privacy = 'friends' AND content_title LIKE '%".$search."%'
UNION
SELECT uid FROM contents WHERE uid IN (
SELECT DISTINCT ( Z.fid ) AS fid FROM (
SELECT uid, fid FROM friends WHERE uid IN (
...
AND privacy = 'fof' AND content_title LIKE '%".$search."%' AND uid <>$id
UNION
SELECT uid FROM contents WHERE privacy = 'public' AND content_title LIKE '%".$search."%')");
